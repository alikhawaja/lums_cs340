...
(NAME = WoodgroveBank,
 FILENAME = '/var/opt/mssql/data/WoodgroveBank.mdf',
	SIZE = 10,
    MAXSIZE = 50,
    FILEGROWTH = 5)
LOG ON
(NAME = Test_DB_Log,
 FILENAME = '/var/opt/mssql/data/WoodgroveBank.ldf',
	SIZE = 5MB,
    MAXSIZE = 25MB,
    FILEGROWTH = 5MB);
GO
...
/* Test to see if records were inserted.
** They were not inserted because the syntax error terminated the batch.
*/

SELECT * FROM Accounting.BankAccounts

/* Fix the word VALUSE and try again.
** This time the records should be inserted.
*/
...
-- This is because run-time errors only terminate the statement,
-- not the batch.
...
-- The statements up to this point have been ad-hoc queries. 
-- To make our code reusable, we will create a stored procedure.

-- Also notice the SET NOCOUNT ON statement
-- this will turn off counting rows affected.
